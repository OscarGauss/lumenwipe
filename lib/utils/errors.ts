const RESULT_CODE_MESSAGES: Record<string, string> = {
  tx_bad_auth: "The transaction signature does not meet the account's authorization requirements. Make sure you're using the correct secret key.",
  ...
  op_low_reserve: "Insufficient XLM balance to cover the minimum reserve requirement after this operation.",
  op_sell_no_issuer: "The asset issuer account no longer exists - sending balance back is not possible.",
  ...
  op_no_destination: "The destination account does not exist on this network. Make sure the destination address is funded before merging.",
  op_malformed: "The operation is malformed. Check that all parameters are valid.",
  op_already_exist: "The destination account already exists (AccountMerge cannot merge into itself).",
...
export class NoConversionPathError extends Error {
  constructor(asset: string) {
    super(`No conversion path found for ${asset}. You may need to send the balance back to the issuer.`);
